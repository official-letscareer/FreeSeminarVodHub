...
- [ ] **1.0 프로젝트 초기 설정 및 기본 구조** (Push 단위)
...
- [ ] **2.0 유저 인증 (로그인 + 챌린지 검증)** (Push 단위)
    ...
    - [ ] 2.2 챌린지 검증 API 프록시 작성 (커밋 단위)
        ...
        - [ ] 2.2.1 테스트 코드 작성 (`__tests__/api/auth.test.ts`)
        - [ ] 2.2.2 테스트 실행 및 검증
        - [ ] 2.2.3 오류 수정 (필요 시)

    - [ ] 2.3 로그인 페이지 UI 구현 (커밋 단위)
        - `src/app/login/page.tsx`
            ...
            - 검증 성공 시 `/vod`로 리다이렉트
            - 모바일 반응형
        - [ ] 2.3.1 UI 렌더링 및 폼 제출 동작 확인
        - [ ] 2.3.2 오류 수정 (필요 시)

    - [ ] 2.4 인증 미들웨어 구현 (커밋 단위)
        - `src/middleware.ts`
