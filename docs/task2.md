...
- [x] **5.0 전화번호 입력 포맷팅** (Push 단위)

    - [x] 6.1 로그인 페이지 전화번호 자동 하이픈 삽입 (커밋 단위)
        ...
        - [x] 6.1.1 포맷팅 동작 확인 (입력, 백스페이스, 붙여넣기)

    - [x] 6.2 예외 유저 추가 폼 전화번호 자동 하이픈 삽입 (커밋 단위)
        - `src/app/admin/vod/page.tsx` 수정
...
- [x] **6.0 영상 리스트뷰 / 썸네일뷰 전환** (Push 단위)

    - [x] 7.1 리스트뷰 컴포넌트 구현 (커밋 단위)
        - `src/components/vod-list-item.tsx` 생성
            - 좌측 썸네일 + 우측 제목/제작일/설명 레이아웃
            - 클릭 시 `/vod/[id]`로 이동
            - 호버 효과 및 썸네일 폴백 처리
        - [x] 7.1.1 컴포넌트 렌더링 확인

    - [x] 7.2 VOD 목록 페이지에 뷰 전환 기능 추가 (커밋 단위)
        - `src/app/vod/page.tsx` 수정
            - 목록 상단에 그리드/리스트 아이콘 버튼 추가
            - 상태(`viewMode: 'grid' | 'list'`)로 현재 뷰 관리
            - `localStorage`에 뷰 설정 저장 (재방문 시 유지)
            - 썸네일뷰: 기존 `VodCard` 그리드
            - 리스트뷰: `VodListItem` 세로 목록
        - [x] 7.2.1 빌드 확인
